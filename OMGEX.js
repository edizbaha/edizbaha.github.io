window.OMGEX = function (options) {
    ...
    if(options.karma) {
        ...
        fetch(`https://irc.social.lol/api/karma.php?address=${options.karma}`)
            ...
            .then(res => {
                let karma = res.karma;
                details.innerHTML = details.innerHTML + `<div id="karma"><img src="https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Travel%20and%20places/Star.png" alt="Star" width="25" height="25" /> ${karma} </div>`;
            });
    }
    if(options.birthday) {
        let details = document.querySelector("#details");
